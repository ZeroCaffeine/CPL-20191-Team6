...
const App = {
 ...
 loadRoomCount: async function() {
   const { getRoomCount } = this.voting.methods;
   var roomCount = await getRoomCount().call();
   $("#roomCount").html("Room count : " + roomCount);
 },
 ...
 loadRoomName: async function() {
   const { getRoomName } = this.voting.methods;
   
   $("#loadRoomName").html("load room name : start");
   var roomName = await getRoomName(roomNumber).call();
 
   $("#roomName").html(this.web3.utils.hexToAscii(roomName));
   $("#loadRoomName").html("load room name : finished");
 },

 loadVoteDate: async function() {
   const { getVoteDate } = this.voting.methods;
   
   $("#loadVoteDate").html("load vote date : start");
   var voteDate = await getVoteDate(roomNumber).call();
 
   $("#voteDate").html(this.web3.utils.hexToAscii(voteDate));
   $("#loadVoteDate").html("load vote date : finished");
 },
